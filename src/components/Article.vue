...
function ItemClick(index, node) {
  if (node.childNodes.length == 0) {
    let time = new Date();
    if (item[1].toString() == index.toString()) {
      if (time - item[0] < 500) {//双击
        let url = ['./Blog', props.cont_meta['name'], ...index].join('/');
        open(url);
      }
    }
    else {
  ...
  <div>
      ...
      <template #extra>
        <el-tooltip placement="left">
          <template #content>
            ...
            <span v-show="props.cont_refresh.length > 1"><br>XRateLimit重置时间：{{ TransTime(props.cont_refresh[1]) }}</span>
    ...
    <div v-loading="props.isLoading" :class="props.type" element-loading-text="加载中...">
      ...
      <div v-show="props.showAddition">
        ...
        <el-col :span="5" v-show="props.cont_meta.struct">
